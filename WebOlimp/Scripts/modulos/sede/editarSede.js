...
function Registrar() {
    ...
    try {
        ...
        $.ajax({
            url: rutaPrincipal + `Sede/PutEditarSede/${_id}`,
            method: 'PUT',
            data: datos,
            success: function (data, textStatus, xhr) {
                ...
                if (xhr.status == 200) {
                    verMensaje('', "Se ha editado correctamente la sede", 'green',
                        'Ok', function () { location.href = `${rutaPrincipal}Sede/ListaSede` });
...
function Cancelar() {
    verMensaje('', "¿Deseas cancelar la edición de la sede?", 'orange',
        'Si', function () { location.href = `${rutaPrincipal}Sede/ListaSede` }, function () { }, 'No');
}
...
function Detalle() {
    try {
        showLoader();
        $.ajax({
            url: rutaPrincipal + `Sede/GetDetalleSede/${_id}`,
            method: 'GET',
            success: function (data, textStatus, xhr) {
                hideLoader();
            ...
            error: function (xhr, textStatus) {
                ...
                verMensaje('', "No se pudo recuperar los datos de la sede", 'orange',
                    'Ok', function () { location.href = `${rutaPrincipal}Sede/ListaSede` }, function () { location.href = `${rutaPrincipal}Sede/ListaSede` });
            }
